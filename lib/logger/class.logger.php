...
class Logger extends Singleton
{
	...
	private $entries = array(), $cli = false, $catchable_error_type, $init_time;

	public function __construct()
	{
		$this->catchable_error_type = E_ALL | E_STRICT;
		$this->init_time = microtime();
	}

	/**
	 * Clear logged content
	 */
	...
	{
		FileSystem::writeFile($path, print_r($this->entries, true), true);
	}
	 ...
	 * @param string $type Type of entry (from LoggerEntry enumeration)
	 * @param string $sender Message sender
	 * @param string $message Content of the message to be logged
	 */
	public function log($type, $sender, $message)
	{
		$this->entries[] = array($type, $sender, $message, microtime(), time());
	}
	...
	public function buildConsole()
	{
		...
		$html .= OrionTools::linef('<a href="" onclick="logger_js_show_panel(\'logger_block_curl\');  return false;"><span class="logger_icon logger_icon_cache"></span>Curl</a>');
		...
		$html .= OrionTools::linef('<table class="logger_togglableBoth">');
		...
		// CURL
		//
		$html .= OrionTools::linef('<div class="logger_block" id="logger_block_curl" style="display: none;">');
		$html .= OrionTools::linef('<h3><span class="logger_icon logger_icon_cache"></span> Curl messages</h3>');
		$html .= OrionTools::linef('<table class="logger_togglableBoth">');
		$html .= OrionTools::linef('<tr><th>Key</th><th>Source</th><th>Time</th></tr>');
		foreach ($this->getEntriesByType(LoggerEntry::CURL) as $entry)
		{
			$html .= OrionTools::linef('<tr><td>' . $entry[0] . '</td><td>' . $entry[1] . '</td><td>' . $this->microDateTime($entry[3]) . '</td></tr>');
			$html .= OrionTools::linef('<tr><td colspan="3">' . $entry[2] . '</td></tr>');
		}
