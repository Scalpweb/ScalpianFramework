...
class Table
{
	...
	static public function getStaticTableName()
	{
		return static::$tbl_name;
	}

	static public function getStaticDatabaseName()
	{
	                       ...
	                       $ascii = false, $unicode = false, $enum = '', $set = '')
	{
		if (isset($this->rows[$_name]))
			throw(new RowAlreadyExistsException("A row named [" . $_name . "] already exists."));
	...
	/**
	 * Return a record from cache for the table given a certain field value
	 *
	 * @param $key
	 * @param $ttl
	 * @param $row
	 ...
	 */
	static public function fetchOneFromCacheBy($key, $ttl, $row, $value, $result_type = QueryResultType::RECORD_OBJECT, $eagerLoading = array())
	{
		$key = 'record.'.$key;
		$cached = Cache::getInstance()->get($key);
		if($cached === false)
		{
			$cached = self::fetchOneBy($row, $value, $result_type, $eagerLoading);
			Cache::getInstance()->set($key, $cached, $ttl);
		}
		return $cached;
	}
	...
	/**
	 * Return records from cache for the table given a certain field value
	 *
	 * @param $key
	 * @param $ttl
	 * @param $row
	 ...
	 * @return mixed|null
	 */
	static public function fetchAllFromCacheBy($key, $ttl, $row, $value, $result_type = QueryResultType::RECORD_OBJECT, $eagerLoading = array())
	{
		$key = 'record.all.'.$key;
		$cached = Cache::getInstance()->get($key);
		if($cached === false)
		{
			$cached = self::fetchAllBy($row, $value, $result_type, $eagerLoading);
			Cache::getInstance()->set($key, $cached, $ttl);
		}
		return $cached;
	}
	...
	{
		$query = $this->query()->addSelect('COUNT(id) as cnt')->execute(QueryResultType::PDO_ARRAY, true);
		return $query['cnt'];
	}
