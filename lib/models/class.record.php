...
class Record
{
	...
	protected $instanceId, $name, $tableName, $table, $values = array(), $relations = array(), $initial_values = array(), $initial_relations = array();
		...
		foreach ($objects as $instance)
		{
			if (is_array($eagerLoading))
			{
				foreach ($eagerLoading as $key => $value)
				{
					...
					$instance->$relationName = $relationObject->formatEagerLoadingResult(
						static::createObjectFromArrayWithEagerLoading(
							new $objectName(),
							$array,
							$value,
							$relationAlias,
	...
	/**
	 * Insure table object is not also serialized
	 * @return array
	 */
	public function __sleep()
	{
		return array('instanceId', 'name', 'tableName', 'values', 'relations', 'initial_values', 'initial_relations');
	}
	...
	{
		$this->table = Table::getTable(strtolower(get_class($this)));
	}
