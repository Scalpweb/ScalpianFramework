...
class Module
{
	 ...
	 * @return string
	 */
	public function getHtmlFinalCode()
	{
	 ...
	 * @param $code
	 */
	public function setHtmlFinalCode($code)
	{
		$this->finalCode = $code;
	}
	 ...
	 * Replace the 'insertBlock' anchors by an Orion tag
	 */
	public function insertCachableBlock($blockName, $arguments, $cacheName, $cacheTTl)
	{
		$html = Cache::getInstance()->get($cacheName);
		if ($html === false)
		{
			ob_start();
			$block = new Block($this, $blockName);
			...
			ob_end_clean();
			Cache::getInstance()->set($cacheName, $html, $cacheTTl);
		}
		echo $html;
	}
	...
	public function getAnchor($key)
	{
		echo $this->anchors[$key];
	}
