...
class User extends Singleton
{
	...
	public static function doesPasswordMatch($value, $encrypted)
	{
		return self::encryptPassword($value) == $encrypted;
	}
	...
	private function saveSession($id, $password)
	{
		...
		Session::getInstance()->set('User/key', self::encryptPassword($id . '-' . $password));
		if ($this->use_cookie)
		{
	...
	public function validateSession()
	{
		...
		if ($this->use_database)
		{
			...
			try
			{
				$user = Table::getTable($userclassname)->fetchOneFromCacheBy('user.cached.'.$this->user_id, 1800, 'id', $this->user_id, QueryResultType::RECORD_OBJECT);
				if (
					!is_null($user) &&
					$user->id === $this->user_id &&
					$this->user_key === self::encryptPassword($user->id . '-' . $user->password)
				)
				{
					$this->isRegistered = true;
			...
			$this->user_key === self::encryptPassword($this->username . '-' . $this->password)
		)
		{
			$this->isRegistered = true;
		}
	 ...
	 * @return Record User table record or null
	 */
